...
% == NOT FUNCTIONAL DO NOT TURN ON ======================================================
% % -- Add Dihedral ---------------------------------------------------------Add Dihedral
% for m = 1:len_y_pts
%     [~,        z_edges_VEC(m)]      = add_dihedral(y_VEC(m), z_edges_VEC(m),      dihedral);
%     [~,        z_lead_vort_VEC(m)]  = add_dihedral(y_VEC(m), z_lead_vort_VEC(m),  dihedral);
%     [y_VEC(m), z_trail_vort_VEC(m)] = add_dihedral(y_VEC(m), z_trail_vort_VEC(m), dihedral);
% end % m = 1:len_y_points
% 
% for m = 1:N/2
%     [~,                 z_ctrl_lead_VEC(m)]  = add_dihedral(y_ctrl_pts_VEC(m), z_ctrl_lead_VEC(m),  dihedral);
%     [y_ctrl_pts_VEC(m), z_ctrl_trail_VEC(m)] = add_dihedral(y_ctrl_pts_VEC(m), z_ctrl_trail_VEC(m), dihedral);
% end % m = 1:N/2
% % -------------------------------------------------------------------------Add Dihedral
% REQUIRES NORM WORK DOWNSTREAM AND MORE ACCURATE NUMERICAL ANALYSIS!!!!!!!
% =======================================================================================
...
% Vel_mat_w_wash = zeros(N, 3);
% Vel_mat = U_inf_VEC + cQnm;
% Velocity at each panel
% for m = 1:N
%     Vel_mat_w_wash(m, :) = U_inf_VEC(m, :) + cQnm(m, :);
% end  % m = 1:N
Vel_mat_w_wash = U_inf_VEC + cQnm;
Vel = U_inf_vec + cQnm;
% -------------------------------------------------------------------------RN, Qmn w/ midpoint Vn

% -- Generate Axial, Y-Force, Normal --------------------------------------Axial, Y-Force, Normal
F_n = zeros(N,3);

for m = 1:N
    F_n(m, :) = rho * Circ_n(m) * cross( Vel_mat_w_wash(m, :), Rn(m, :));
end % m = 1:N

Axial   = sum(F_n(:,1)); % force x
Y_force = sum(F_n(:,2)); % force y
Normal  = sum(F_n(:,3)); % force z

% -------------------------------------------------------------------------Axial, Y-Force, Normal
...
Wf_forces = wf_bf_transform_mat\[Axial; Y_force; Normal];
Drag = Wf_forces(1); Side = Wf_forces(2); Lift = Wf_forces(3);
fprintf("Axial = %f,\t Y-force = %f,\t Normal = %f\n", Axial, Y_force, Normal);
